...
<script type="text/javascript">
	function selectElement() {
		...
		var materialIdString = $('input:radio[name="elementidentity"]:checked').parent().next().text();
		materialIdString = materialIdString.replace("\n", "").replace("\n","").trim();
		var thicknessString = $('input:radio[name="elementidentity"]:checked').parent().next().next().text();
		thicknessString = thicknessString.replace("\n", "").replace("\n","").trim();
		var colorString = $('input:radio[name="elementidentity"]:checked').parent().next().next().next().text();
		colorString = colorString.replace("\n", "").replace("\n","").trim();
		var lengthString = $('input:radio[name="elementidentity"]:checked').parent().next().next().next().next().text();
		...
		var stockString = $('input:radio[name="elementidentity"]:checked').parent().next().next().next().next().next().text();
		stockString = stockString.replace("\n", "").replace("\n","").trim();
		
		var materialtype = $('input:radio[name="elementidentity"]:checked').parent().next().next().next().next().next().next().text();
		...
		if (val == null) {
			...
			$('#'+materialtypeElementObject, window.opener.document).val(materialtype);
					...
					<table
						...
						<thead>
							<tr>
								...
								<th><sp:message code="label.materialtype" /></th>
							</tr>
						</thead>
						...
						<c:forEach var="element" items="${elementList}" varStatus="loop">
							<tr>
								...
								<td><c:out value="${element.materialtype}" /></td>
